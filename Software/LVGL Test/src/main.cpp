 ...
 *     -> set #define LV_COLOR_DEPTH 16            (should be already set to the
 *correct value)
 *     -> set #define LV_DISP_DEF_REFR_PERIOD 15   (change this from 30ms to
 *15ms for more reactive display)
 *     -> set #define LV_USE_PERF_MONITOR 1        (if you want to to show the
 *FPS counter)
 *     -> set #define LV_FONT_MONTSERRAT_12  1     (should be already set to the
 *correct value)
...
#define PIN_TOUCH_IRQ \
  19  // optional, set this only if the touchscreen is connected on the same SPI
      // bus
#define PIN_TOUCH_CS \
  18  // optional, set this only if the touchscreen is connected on the same spi
      // bus
...
void my_disp_flush(lv_disp_drv_t* disp, const lv_area_t* area,
                   lv_color_t* color_p) {
  const bool redraw_now =
      lv_disp_flush_is_last(disp);  // check if when should update the screen
                                    // (or just buffer the changes).
  tft.updateRegion(redraw_now, (uint16_t*)color_p, area->x1, area->x2, area->y1,
                   area->y2);  // update the interval framebuffer and then
                               // redraw the screen if requested
  lv_disp_flush_ready(disp);   // tell lvgl that we are done and that the lvgl
                               // draw buffer can be reused.
}
...
void setup() {
  ...
  tft.setDiffGap(
      4);  // with have large 8K diff buffers so we can use a small gap.
  tft.setVSyncSpacing(1);   // lvgl is already controlling framerate: we just
                            // set this to 1 to minimize screen tearing.
