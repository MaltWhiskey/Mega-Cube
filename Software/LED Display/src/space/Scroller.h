...
class Scroller : public Animation {
 ...
 public:
  ...
  void draw(float dt) {
    ...
    const uint16_t blank_lines = 2;

    uint16_t text_lines = (CHARSET1_FRAME_HEIGHT + blank_lines) * txt.length();
    while (line_angle > -line_angle_adj) {
      uint16_t text_offset = pixel_line % text_lines;
      uint16_t t =
          match_char(txt[text_offset / (CHARSET1_FRAME_HEIGHT + blank_lines)]);
      if (t == 0xffff) {
        if (line_angle > 90.0f) {
          text_rotation = -95.0f;
      ...
      uint16_t y = pixel_line++ % (CHARSET1_FRAME_HEIGHT + blank_lines);
      if (y < CHARSET1_FRAME_HEIGHT) {
        Quaternion q = Quaternion(line_angle, Vector3::X);
        for (uint8_t x = 0; x < CHARSET1_FRAME_WIDTH; x++) {
          uint32_t data = charset1_data[t][y * CHARSET1_FRAME_WIDTH + x];
          if (data & 0xff000000) {
            ...
            voxel(pixel, c, Vector3(-7.5, -7.5, +7.5));
          }
        }
      }
